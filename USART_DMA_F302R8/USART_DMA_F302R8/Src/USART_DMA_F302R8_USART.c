...
#include "USART_DMA_F302R8_USART.h"
extern void** USART_DMA_F302R8_block_outtbptr; //External array definition of output pointers for blocks 
...
void  USART2_SendDataFcn(struct USART_TxDataLinkTypeDef* ptInfo)
{
   ...
   if(ptInfo->txStatus != USART_TX_ON && ptInfo->nb2Send)
   {
       ...
       if(ptInfo->nb2Send <= L_sizeMsg)
       {
           ...
           /* USART2 DMA send mode*/
           HAL_UART_Transmit_DMA(&huart2, ptInfo->pt_Tx_R, ptInfo->nb2Send);
       }
       else
       {
           ptInfo->nbSent = L_sizeMsg;
           /* USART2 DMA send mode*/
           HAL_UART_Transmit_DMA(&huart2, ptInfo->pt_Tx_R, L_sizeMsg);
       }
   }
}
...
void USART_Initialize(void){
   USART_DMA_F302R8_block_outtbptr[0] = (uint16_t*)&USART2_RxDataLink.nbRcv; //Set first output block pointer
   USART_DMA_F302R8_block_outtbptr[1] = (uint8_t*)&USART2_RxDataLink.ptRcv; //Set second output block pointer
   USART2_Rx_Initialization(); //Initialize USART2 to receive
   USART_DMA_F302R8_block_outtbptr[5] = (uint16_t*)&USART2_TxDataLink.nbSent; //Set output block pointer
