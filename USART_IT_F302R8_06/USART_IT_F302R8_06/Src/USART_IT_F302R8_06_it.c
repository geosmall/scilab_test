...
/* ---- Headers ---- */
#include "USART_IT_F302R8_06_it.h"
   ...
   *******************************************************************************/
   ...
   {
    if (*(uint16_t*)USART_IT_F302R8_06_block_outtbptr[2] != 0)
    {
     /* Wait for end of current send data*/
     while(USART2_TxDataLink.txStatus == USART_TX_ON);
     /* Copy data to send */
     USART2_CopyDataFcn(*(uint8_t**)USART_IT_F302R8_06_block_outtbptr[3], *(uint16_t*)USART_IT_F302R8_06_block_outtbptr[2], &USART2_TxDataLink);
    }
